...
def gen_all_memory_tests():
	...
	for probR in range(1,5,2):
		r = 0.1*probR
		mng.writeMemoryTickMilestone(path+"memoryTickMilestone["+str(r)+"].mdp")
    	mng.writeMemoryRefreshMilestone(path+"memoryRefreshMilestone["+str(r)+"].mdp")
    ...
    for bits in range(3,11,2):
    	for probF in range(1,5,2):
    		for probR in range(1,5,2):
    			r = 0.1*probR
    			f = 0.1*probF   			
    			mfg.writeMemoryTickMilestone(path+"memoryTickMilestone["+str(bits)+"-"+str(r)+"-"+str(f)+"].mdp")
    			mfg.writeMemoryRefreshMilestone(path+"memoryRefreshMilestone["+str(bits)+"-"+str(r)+"-"+str(f)+"].mdp")
...
def gen_all_nmr_tests():
    ...
    for modules in range(3,11,2):
    	for probF in range(1,5,2):
    		f = 0.1*probF
    		nfg.writeNMR(path+"nmr["+str(modules)+"-"+str(f)+"].mdp")
    ...
    for modules in range(3,11,2):
    	for probP in range(1,5,2):
    		for probV in range(1,5,2):
    			p = 0.1*probP
    			v = 0.1*probV   			
    			pfg.writeNMR(path+"nmr-proc-mem["+str(modules)+"-"+str(p)+"-"+str(v)+"].mdp")
